...
function reverseInteger(num) {
  return Number.parseInt(String(num).split('').reverse().join(''), 10);
}
...
function isCreditCardNumber(ccn) {
  const strippedCCN = String(ccn).replace(/[^\d]/g, '');
  if (strippedCCN.length < 13 || strippedCCN.length > 19) {
    return false;
  }
  let sum = 0;
  let shouldDouble = false;
  for (let i = strippedCCN.length - 1; i >= 0; i -= 1) {
    let digit = parseInt(strippedCCN[i], 10);
    if (shouldDouble) {
      digit *= 2;
      if (digit > 9) {
        digit -= 9;
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < firstPath.length; i += 1) {
    for (let j = 1; j < pathes.length; j += 1) {
      const otherPath = pathes[j].split('/');
      if (firstPath[i] !== otherPath[i]) {
        pathEqual = false;
        break;
      }
    }
    if (pathEqual) {
      commonPath += firstPath[i].concat('/');
    } else {
      break;
    }
  }
